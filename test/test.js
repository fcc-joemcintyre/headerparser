...
describe ("test server", function () {
    ...
    it ("should return 200 with instructions", function (done) {
      request.get ("http://localhost:3000/", function (err, res, body) {
        ...
        if (res.statusCode === 200) {
          if (body.startsWith ("<h1>Request Header Parser Service</h1>")) {
            return done ();
          }
        }
  ...
  describe ("valid request", function () {
    it ("should return JSON object with natural, unix formats", function (done) {
      request.get ("http://localhost:3000/api/client", function (err, res, body) {
        if (err) { return done (err); }
        if (res.statusCode === 200) {
          try {
            // can only test that JSON came back, since test client is not a browser
            JSON.parse (body);
            return done ();
          } catch (e) {
